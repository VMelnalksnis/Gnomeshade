...
public sealed class TransactionSchedulesController
	: CreatableBase<TransactionScheduleRepository, TransactionScheduleEntity, TransactionSchedule, TransactionScheduleCreation>
{
		...
		UserEntity user)
	{
		...
		return await Repository.UpdateAsync(schedule) switch
		{
			1 => NoContent(),
	...
	/// <inheritdoc />
	protected override async Task<ActionResult> CreateNewAsync(Guid id, TransactionScheduleCreation creation, UserEntity user)
	{
		...
		_ = await Repository.AddAsync(schedule);
