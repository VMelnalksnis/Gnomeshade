...
public sealed class AccountRepository : NamedRepository<AccountEntity>
{
	...
	/// <inheritdoc />
	protected override string FindSql => $"WHERE a.id = @id AND ownerships.user_id = @ownerId {_accessSql};";

	/// <inheritdoc />
	protected override string UpdateSql => Queries.Account.Update;

	/// <inheritdoc />
	protected override string NameSql => $"WHERE a.normalized_name = @name AND ownerships.user_id = @ownerId {_accessSql};";
	...
	{
		var sql = $"{SelectSql} WHERE a.iban = @iban AND ownerships.user_id = @ownerId {_accessSql};";
	...
	{
		var sql = $"{SelectSql} WHERE a.bic = @bic AND ownerships.user_id = @ownerId {_accessSql};";
	...
	{
		var sql = $"{SelectSql} WHERE a.disabled_at IS NULL AND aic.disabled_at IS NULL AND ownerships.user_id = @ownerId {_accessSql} ORDER BY a.created_at;";
