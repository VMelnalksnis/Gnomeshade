...
public sealed class TransactionRepository : Repository<TransactionEntity>
{
	...
	/// <inheritdoc />
	protected override string FindSql => $"WHERE t.id = @id AND ownerships.user_id = @ownerId {_accessSql};";
	...
	{
		var sql = $"{SelectSql} WHERE t.import_hash = @importHash AND ownerships.user_id = @ownerId {_accessSql};";
	...
	public async Task<List<TransactionEntity>> GetAllAsync(
		...
		CancellationToken cancellationToken = default)
	{
		var sql = $"{SelectSql} WHERE t.date >= @from AND t.date <= @to AND ownerships.user_id = @ownerId {_accessSql} ORDER BY t.date DESC";
